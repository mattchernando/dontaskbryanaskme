...
const CITIES = [
  ...
  {name:"Hazelwood",state:"MO",lat:38.771,lon:-90.371,pop:25000,noRadar:true},
  {name:"Bridgeton",state:"MO",lat:38.751,lon:-90.428,pop:11500,noRadar:true},
  ...
  {name:"Lambert Intl ✈",state:"MO",lat:38.748,lon:-90.370,pop:0,noRadar:true},
...
// ── Threat-tier ranking + plain-English peak time formatting ─────────────────
const THREAT_TIER = { LOW:0, ELEVATED:1, MODERATE:2, HIGH:3, EXTREME:4 };
const tierOf = lv => THREAT_TIER[lv] ?? 0;
function formatPeakTime(date) {
  if (!date) return "";
  const now = new Date();
  const isToday = date.toDateString() === now.toDateString();
  const hour = date.toLocaleTimeString("en-US", { hour: "numeric", hour12: true });
  return isToday ? hour : date.toLocaleDateString("en-US", { weekday: "short" }) + " " + hour;
}

// ── Forecast-aware analysis ──────────────────────────────────────────────────
// Runs analyze() on every forecast hour and returns { current, peak, peakTime, peakIdx, hIdx }.
// peak/peakTime are null when the worst hour IS the current hour.
function analyzeForecast(cur, hrly, alertFeatures) {
  if (!hrly || !hrly.time || !hrly.time.length) return null;
  const hIdx = findCurrentHourIdx(hrly.time);
  const current = analyze(cur, hrly, hIdx, alertFeatures);
  if (!current) return null;

  let peakIdx = hIdx;
  let peakScore = current.stp;
  for (let i = hIdx + 1; i < hrly.time.length; i++) {
    const fa = analyze(cur, hrly, i, alertFeatures);
    if (fa && fa.stp > peakScore) {
      peakScore = fa.stp;
      peakIdx = i;
    }
  }

  if (peakIdx === hIdx) return { current, peak: null, peakTime: null, peakIdx, hIdx };
  const peak = analyze(cur, hrly, peakIdx, alertFeatures);
  const peakTime = new Date(hrly.time[peakIdx]);
  return { current, peak, peakTime, peakIdx, hIdx };
}

// Build the AI assessment string. If a meaningful upgrade is forecast later, append a forward-looking sentence.
function buildAssessment(current, peak, peakTime) {
  if (!current) return "";
  let s = current.note;
  if (peak && peakTime && tierOf(peak.lv) > tierOf(current.lv)) {
    const t = formatPeakTime(peakTime);
    s += ` Conditions ramp up by ${t}: ${peak.lv} risk projected — CAPE ${Math.round(peak.cape)} J/kg, ${Math.round(peak.dlShear)} mph shear, ${peak.prob}% tornado probability.`;
  }
  return s;
}
...
// ── SVG Radar Map ────────────────────────────────────────────────────────────
function RadarMap({a, cities, radarHost, radarPath, showCone, zoom}) {
  // Base scale at zoom=1 shows ~100mi. zoom<1 = wider view, zoom>1 = closer.
  const PL_BASE = 117, PO_BASE = 107;
  const PL = PL_BASE * zoom, PO = PO_BASE * zoom;
  const col = a.col;
  const toX = lon => (lon - LON) * PO;
  const toY = lat => -(lat - LAT) * PL;

  // Tile zoom scales with map zoom. RainViewer radar tiles top out at z=7;
  // anything higher returns the "Zoom Level Not Supported" placeholder.
  const TZOOM = Math.min(7, Math.max(5, Math.round(6 + Math.log2(zoom))));

  // ── Tile math ──
  const lon2tile = (ln, z) => Math.floor((ln + 180) / 360 * (1 << z));
  const lat2tile = (lt, z) => Math.floor((1 - Math.log(Math.tan(lt * Math.PI / 180) + 1 / Math.cos(lt * Math.PI / 180)) / Math.PI) / 2 * (1 << z));
  const tile2lon = (x, z) => x / (1 << z) * 360 - 180;
  const tile2lat = (y, z) => { const n = Math.PI - 2 * Math.PI * y / (1 << z); return 180 / Math.PI * Math.atan(0.5 * (Math.exp(n) - Math.exp(-n))); };

  const latN = LAT + 175 / PL, latS = LAT - 175 / PL;
  const lonW = LON - 205 / PO, lonE = LON + 205 / PO;
  const txMin = lon2tile(lonW, TZOOM), txMax = lon2tile(lonE, TZOOM);
  const tyMin = lat2tile(latN, TZOOM), tyMax = lat2tile(latS, TZOOM);

  const tiles = [];
  if (radarPath && radarHost) {
    for (let tx = txMin; tx <= txMax; tx++) {
      for (let ty = tyMin; ty <= tyMax; ty++) {
        const tN = tile2lat(ty, TZOOM), tS = tile2lat(ty + 1, TZOOM);
        const tW = tile2lon(tx, TZOOM), tE = tile2lon(tx + 1, TZOOM);
        tiles.push({
          url: `${radarHost}${radarPath}/256/${TZOOM}/${tx}/${ty}/6/0_0.png`,
          x: toX(tW), y: toY(tN),
          width: toX(tE) - toX(tW),
          height: toY(tS) - toY(tN),
        });
      }
    }
  }

  // Visible radius in miles (what fits in the viewBox)
  const visMi = Math.round(170 / PL * 69);
  // Cone projection hours scale with view
  const coneHrs = Math.min(5, 2.5 / zoom);

  // ── Cone (only when showCone) ──
  const sr2 = r(a.stDir);
  const dLat = (Math.cos(sr2) * a.stSpd) / 69, dLon = (Math.sin(sr2) * a.stSpd) / 55;
  let conePts = null;
  if (showCone) {
    const left = [], right = [];
    for (let i = 0; i <= 28; i++) {
      const t = (i / 28) * coneHrs;
      const cLa = LAT + dLat * t, cLo = LON + dLon * t;
      const hw = 0.04 + t * 0.10;
      const pLa = -dLon * hw / (a.stSpd / 69 || 0.001);
      const pLo = dLat * hw / (a.stSpd / 55 || 0.001);
      left.push(`${toX(cLo - pLo).toFixed(1)},${toY(cLa - pLa).toFixed(1)}`);
      right.unshift(`${toX(cLo + pLo).toFixed(1)},${toY(cLa + pLa).toFixed(1)}`);
    }
    conePts = [...left, ...right].join(" ");
  }
  // Cone hour markers — pick nice intervals
  const coneStep = coneHrs <= 2 ? 0.5 : 1;
  const coneMarkers = [];
  for (let h = coneStep; h <= coneHrs + 0.01; h += coneStep) coneMarkers.push(Math.round(h * 10) / 10);

  const inPathSet = new Set(cities.map(c => c.name + c.state));
  const mapCities = CITIES.filter(c => {
    if (c.noRadar) return false;
    const d = Math.sqrt(((c.lat - LAT) * 69) ** 2 + ((c.lon - LON) * 53.5) ** 2);
    return d < visMi + 15;
  });

  // Population threshold: zoomed out = major cities only, zoomed in = more detail
  const popThresh = zoom >= 1.5 ? 8000 : zoom >= 1.0 ? 20000 : zoom >= 0.7 ? 40000 : 70000;

  // Ring intervals adapt to view size
  const ringStep = visMi <= 60 ? 15 : visMi <= 120 ? 25 : 50;
  const rings = [];
  for (let ri = ringStep; ri <= visMi + 5; ri += ringStep) rings.push(ri);

  // Grid step in degrees
  const gridStep = zoom >= 1.5 ? 0.25 : zoom >= 0.7 ? 0.5 : 1;
  const gridLines = [];
  for (let d = -5; d <= 5; d += gridStep) gridLines.push(d);

  return (
    <svg viewBox="-200 -170 400 340" style={{width:"100%",height:"100%",background:"#03060e"}}>
      <defs>
        <marker id="arw" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto"><polygon points="0 0,10 3.5,0 7" fill={col}/></marker>
        <radialGradient id="hg" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="#00ff88" stopOpacity="0.35"/><stop offset="100%" stopColor="#00ff88" stopOpacity="0"/></radialGradient>
        <clipPath id="mapClip"><rect x="-200" y="-170" width="400" height="340"/></clipPath>
        ...
        <filter id="labelShadow"><feDropShadow dx="0" dy="0" stdDeviation="1.2" floodColor="#000" floodOpacity="0.85"/></filter>
      </defs>

      {/* Grid lines */}
      {gridLines.map(d => (
        <g key={d} opacity="0.12">
      ...
      {/* Mississippi River */}
      <polyline points={[[-90.17,39.75],[-90.20,39.50],[-90.15,39.17],[-90.19,38.93],[-90.20,38.62],[-90.05,38.25],[-89.95,37.95],[-89.55,37.55],[-89.25,37.10],[-89.17,36.75]].map(p=>`${toX(p[0]).toFixed(1)},${toY(p[1]).toFixed(1)}`).join(" ")}
        stroke="#1a4a6a" strokeWidth="1.2" fill="none" strokeDasharray="5,3" opacity="0.45"/>

      {/* ── RADAR TILES ── */}
      <g clipPath="url(#mapClip)" opacity="0.8">
        {tiles.map((t, i) => (
      ...
      {!radarPath && (
        <text x="0" y="5" fill="#1a3050" fontSize="7" fontFamily="monospace" textAnchor="middle">RADAR LOADING…</text>
      )}

      {/* Range rings */}
      {rings.map(ri => {
        const rla = ri / 69, rlo = ri / 53.5;
        return <ellipse key={ri} cx={toX(LON)} cy={toY(LAT)} rx={rlo*PO} ry={rla*PL} fill="none" stroke="#0d2035" strokeWidth="0.5" strokeDasharray="3,3"/>;
      })}
      {rings.map(ri => (
        <text key={ri+"t"} x={toX(LON)+4} y={toY(LAT)-(ri/69)*PL-2} fill="#1a3a55" fontSize="5" fontFamily="monospace">{ri}mi</text>
      ))}

      {/* ── STORM CONE (conditional) ── */}
      {showCone && conePts && (
        <g>
          <polygon points={conePts} fill={col+"25"} stroke={col} strokeWidth="1" strokeDasharray="4,2"/>
          {coneMarkers.map(h => {
            const cLa = LAT + dLat * h, cLo = LON + dLon * h;
            const prev = Math.max(0, h - coneStep);
            const isLast = h === coneMarkers[coneMarkers.length - 1];
            return (
              <g key={h}>
                <line x1={toX(LON+dLon*prev)} y1={toY(LAT+dLat*prev)} x2={toX(cLo)} y2={toY(cLa)}
                  stroke={col} strokeWidth="1.2" strokeDasharray="5,3" markerEnd={isLast?"url(#arw)":undefined}/>
                <circle cx={toX(cLo)} cy={toY(cLa)} r="3" fill="#040810" stroke={col} strokeWidth="0.8"/>
                <text x={toX(cLo)+4} y={toY(cLa)+3} fill={col} fontSize="5.5" fontFamily="monospace" filter="url(#labelShadow)">
                  +{h%1===0?h+"H":h+"H"}
                </text>
              </g>
            );
      ...
      {/* Cities — collision-aware labels */}
      {(() => {
        const sorted = [...mapCities].sort((a, b) => {
          if (a.home) return -1; if (b.home) return 1;
          const aIp = inPathSet.has(a.name + a.state), bIp = inPathSet.has(b.name + b.state);
          if (aIp && !bIp) return -1; if (!aIp && bIp) return 1;
          return (b.pop || 0) - (a.pop || 0);
        });
        const placed = [];
        const CHAR_W = 3.8, PAD_X = 5, PAD_Y = 4;
        const collides = (bx, by, bw, bh) => placed.some(p =>
          bx < p.x + p.w + PAD_X && bx + bw + PAD_X > p.x &&
          by - bh < p.y + PAD_Y && by + PAD_Y > p.y - p.h
        );

        return sorted.map(c => {
          const ip = inPathSet.has(c.name + c.state);
          const ic = cities.find(x => x.name === c.name && x.state === c.state);
          const dc = c.home ? "#00ff88" : ip ? (ic?.risk === "DIRECT" ? "#ff4422" : ic?.risk === "HIGH" ? "#ff8844" : "#ffcc44") : "#3a6080";
          const eligible = c.home || ip || c.pop >= popThresh;
          const fs = c.home ? 7.5 : ip ? 6.5 : 5.5;
          const cx2 = toX(c.lon), cy2 = toY(c.lat);
          const labelW = c.name.length * CHAR_W * (fs / 6);
          const dotR = c.home ? 5 : ip ? 4 : 2;

          let labelX = null, labelY = null, anchor = "start";
          if (eligible) {
            const gap = dotR + 3;
            const offsets = [
              { x: cx2 + gap, y: cy2 + 2.5, a: "start" },
              { x: cx2 - gap, y: cy2 + 2.5, a: "end" },
              { x: cx2, y: cy2 - gap - 1, a: "middle" },
              { x: cx2, y: cy2 + gap + fs, a: "middle" },
            ];
            for (const o of offsets) {
              const bx = o.a === "end" ? o.x - labelW : o.a === "middle" ? o.x - labelW / 2 : o.x;
              if (!collides(bx, o.y, labelW, fs)) {
                labelX = o.x; labelY = o.y; anchor = o.a;
                placed.push({ x: bx, y: o.y - fs, w: labelW, h: fs });
                break;
              }
            }
          }
          ...
          return (
            <g key={c.name + c.state}>
              {c.home && <circle cx={cx2} cy={cy2} r="14" fill="url(#hg)"/>}
              <circle cx={cx2} cy={cy2} r={dotR}
                fill={dc} stroke={c.home||ip?"#000":"none"} strokeWidth="0.5"
                opacity={ip || c.home ? 1 : 0.5} filter={c.home||ip?"url(#glow)":undefined}/>
              {labelX != null && (
                <text x={labelX} y={labelY}
                  textAnchor={anchor}
                  fill={c.home || ip ? dc : "#6090b0"}
                  fontSize={fs}
                  fontFamily="monospace"
                  fontWeight={c.home || ip ? "bold" : "normal"}
                  opacity={c.home ? 1 : ip ? 0.95 : 0.6}
      ...
      {/* State labels — only when they're visible */}
      {visMi >= 60 && <text x={toX(-91.5)} y={toY(38.0)} fill="#0d2040" fontSize="8" fontFamily="monospace" opacity="0.3">MO</text>}
      {visMi >= 60 && <text x={toX(-89.5)} y={toY(39.3)} fill="#0d2040" fontSize="8" fontFamily="monospace" opacity="0.3">IL</text>}
      {visMi >= 150 && <text x={toX(-88.2)} y={toY(37.0)} fill="#0d2040" fontSize="6" fontFamily="monospace" opacity="0.25">KY</text>}
      {visMi >= 150 && <text x={toX(-90.2)} y={toY(36.0)} fill="#0d2040" fontSize="6" fontFamily="monospace" opacity="0.25">AR/TN</text>}
      {visMi >= 150 && <text x={toX(-87.3)} y={toY(38.2)} fill="#0d2040" fontSize="6" fontFamily="monospace" opacity="0.25">IN</text>}
    </svg>
  );
}
...
export default function App() {
  ...
  const [apiOk,  setApiOk]  = useState({meteo:null, nws:null, radar:null});
  const [updated, setUpd]   = useState(null);
  const [loading, setLoad]  = useState(false);
  const [radarData, setRadar] = useState({host:null, frames:[]});
  const [tab, setTab]       = useState("analysis");
  const width = useWidth();
  const isMob = width < 680;
  const isSm  = width < 440;

  // Compute analysis from current state — including forward scan for peak threat hour today
  const hIdx       = hrly ? findCurrentHourIdx(hrly.time || []) : 0;
  const fc         = analyzeForecast(wx, hrly, aAlerts);
  const a          = fc?.current;
  const peak       = fc?.peak;
  const peakTime   = fc?.peakTime;
  const impact     = a ? citiesInPath(a) : [];
  const col        = a ? a.col : "#00dd66";
  // Banner styling: when peak today is worse than now, paint the border with the peak's color
  // so a "LOW now / HIGH later" situation reads as orange at a glance instead of green.
  const showPeak   = !!(peak && peakTime && a && tierOf(peak.lv) > tierOf(a.lv));
  const bannerCol  = showPeak ? peak.col : col;
  ...
  const fetchAll = useCallback(async () => {
    ...
    // RainViewer Radar — assemble [LIVE, +10min, +20min, +30min] frame list
    let radarOk = false;
    try {
      ...
      const frames = [];
      if (past.length > 0) frames.push(past[past.length - 1]); // current "LIVE" frame
      nowcast.forEach(f => frames.push(f));                    // 0–3 forecast frames (10-min steps, ≤30 min)
      if (frames.length > 0) {
        setRadar({ host: data.host || "https://tilecache.rainviewer.com", frames });
        radarOk = true;
      }
    } catch(e) {
    ...
    setApiOk({ meteo: meteoOk, nws: nwsOk, radar: radarOk });
    setUpd(new Date());
    setLoad(false);
  }, []);
  ...
  return (
    <div style={{background:"#060c1a",minHeight:"100vh",color:S.text,fontFamily:"system-ui,sans-serif",padding:isMob?"8px":"14px",maxWidth:1280,margin:"0 auto"}}>
      ...
      <div style={{background:a&&a.hasTW?"linear-gradient(135deg,#2a0000,#1a0505)":"linear-gradient(135deg,#08102a,#060c1a)",
        ...
        <div>
          ...
          <div style={{display:"flex",gap:10,marginTop:4,flexWrap:"wrap"}}>
            ...
            <StatusDot ok={apiOk.radar} label={apiOk.radar===null?"RADAR: LOADING…":apiOk.radar?"RADAR: LIVE":"RADAR: OFFLINE"}/>
      ...
      {/* THREAT BANNER */}
      {a && (
        <div style={{background:`linear-gradient(90deg,${bannerBg},#060c1a)`,border:`2px solid ${bannerCol}50`,
          borderRadius:8,padding:isMob?"10px 12px":"12px 16px",marginBottom:8,
          boxShadow:`0 0 16px ${bannerCol}20`}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
              {/* NOW */}
              <div>
                <div style={{color:"#1a4060",fontFamily:"monospace",fontSize:9,letterSpacing:1}}>{showPeak ? "RIGHT NOW" : "AI TORNADO THREAT"}</div>
              ...
              {/* PEAK TODAY (only when worse than now) */}
              {showPeak && (
                <div style={{borderLeft:`1px solid ${peak.col}40`,paddingLeft:12}}>
                  <div style={{color:"#1a4060",fontFamily:"monospace",fontSize:9,letterSpacing:1}}>↗ PEAK TODAY</div>
                  <div style={{color:peak.col,fontFamily:"monospace",fontSize:isMob?20:24,fontWeight:"bold",
                    letterSpacing:2,textShadow:`0 0 14px ${peak.col}60`,lineHeight:1.1}}>
                    {peak.lv}
                  </div>
                  <div style={{color:peak.col,fontFamily:"monospace",fontSize:10,opacity:0.85}}>at {formatPeakTime(peakTime)} · {peak.prob}% prob</div>
                </div>
              )}
              {/* Prob/STP — reflects current hour */}
            ...
            {!isMob&&(
              <div style={{background:"#060c18",border:`1px solid ${bannerCol}30`,borderRadius:6,padding:"9px 12px",maxWidth:460,flex:1}}>
                <div style={{color:"#1a4060",fontFamily:"monospace",fontSize:9,marginBottom:4}}>AI ASSESSMENT</div>
                <div style={{color:"#7a9ab0",fontFamily:"monospace",fontSize:10,lineHeight:1.6}}>{assessment}</div>
              </div>
            )}
          </div>
          {isMob&&<div style={{marginTop:9,color:"#5a7a90",fontFamily:"monospace",fontSize:10,lineHeight:1.6}}>{assessment}</div>}
        </div>
      )}
      ...
      {/* NEXT 10 HOURS */}
      <HourlyPanel hrly={hrly} hIdx={hIdx} mob={isMob}/>
      ...
      {!isMob ? (
        <>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
            <AnalPanel a={a}/>
            <MapPanel  a={a} impact={impact} col={col} radarData={radarData}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        ...
        <div style={{background:S.bg,border:`1px solid ${S.border}`,borderTop:"none",borderRadius:"0 0 8px 8px",marginBottom:8}}>
          ...
          {tab==="map"     &&<MapPanel     a={a} impact={impact} col={col} mob radarData={radarData}/>}
      ...
      <div style={{color:"#0d1e2e",fontFamily:"monospace",fontSize:8,textAlign:"center",marginTop:6,lineHeight:1.6}}>
        DATA: Open-Meteo (ECMWF/GFS) + NOAA NWS + RainViewer NEXRAD · METHODS: CAPE · LI · CIN · Bulk Wind Shear · SRH · STP · Bunkers RM
...
function MapPanel({a, impact, col, mob, radarData}) {
  ...
  // ── Forecast frame animation ──
  // radarData.frames = [LIVE, +10min, +20min, +30min] (some may be absent)
  const frames = radarData?.frames || [];
  const hasForecast = frames.length > 1;
  const [frameIdx, setFrameIdx] = useState(0);
  const [playing, setPlaying] = useState(true);
  const safeIdx = frames.length > 0 ? Math.min(frameIdx, frames.length - 1) : 0;
  const currentFrame = frames[safeIdx];

  // Reset to LIVE whenever a fresh batch of frames arrives
  useEffect(() => { setFrameIdx(0); }, [frames.length, frames[0]?.time]);

  // Auto-advance through frames when playing (slight pause on the LIVE frame, faster through forecast)
  useEffect(() => {
    if (!playing || !hasForecast) return;
    const dwell = safeIdx === 0 ? 1400 : 800; // hold LIVE longer
    const t = setTimeout(() => {
      setFrameIdx(i => (i + 1) % frames.length);
    }, dwell);
    return () => clearTimeout(t);
  }, [playing, hasForecast, safeIdx, frames.length]);

  // Plain-English label for current frame
  const baseTime = frames[0]?.time;
  const offsetMin = currentFrame && baseTime ? Math.round((currentFrame.time - baseTime) / 60) : 0;
  const isLive = offsetMin <= 0;
  const timeLabel = isLive ? "LIVE" : `IN ${offsetMin} MIN`;
  const timeColor = isLive ? "#00cc66" : offsetMin >= 30 ? "#ff7733" : offsetMin >= 20 ? "#ff9933" : "#ffbb33";
  ...
  const showCone = a.lv === "MODERATE" || a.lv === "HIGH" || a.lv === "EXTREME" || a.hasTW || a.hasTWa;

  const btnStyle = {
    background:"rgba(10,24,48,0.85)",border:"1px solid #1a3a6a",color:"#4499cc",
    borderRadius:5,width:32,height:32,fontFamily:"monospace",fontSize:16,fontWeight:"bold",
  ...
  return (
    <div style={{background:"#03060e",border:`1px solid ${a.hasTW?"#ff220050":S.border}`,borderRadius:mob?0:8,overflow:"hidden",position:"relative",minHeight:mob?280:360}}>
      <RadarMap a={a} cities={impact} radarHost={radarData?.host} radarPath={currentFrame?.path} showCone={showCone} zoom={zoom}/>
      ...
      {/* Zoom level indicator + current frame time badge */}
      <div style={{position:"absolute",top:10,left:8,display:"flex",gap:6,alignItems:"center"}}>
        <div style={{background:"rgba(3,6,14,0.85)",borderRadius:4,padding:"3px 7px",border:"1px solid #0a1a28"}}>
          <span style={{color:"#1a4a60",fontFamily:"monospace",fontSize:9}}>{visMi} MI</span>
        </div>
        {currentFrame && (
          <div style={{background:"rgba(3,6,14,0.9)",borderRadius:4,padding:"3px 8px",border:`1px solid ${timeColor}80`}}>
            <span style={{color:timeColor,fontFamily:"monospace",fontSize:10,fontWeight:"bold",letterSpacing:1}}>{timeLabel}</span>
          </div>
        )}
      </div>

      {/* Forecast scrubber (only when nowcast frames available) */}
      {hasForecast && (
        <div style={{position:"absolute",bottom:30,left:8,right:8,background:"rgba(3,6,14,0.92)",borderRadius:5,padding:"6px 8px",border:`1px solid #0a1a28`,display:"flex",alignItems:"center",gap:8}}>
          <button onClick={() => setPlaying(p => !p)}
            style={{background:"rgba(10,24,48,0.6)",border:"1px solid #1a3a6a",color:"#4499cc",borderRadius:4,width:24,height:20,fontSize:11,cursor:"pointer",padding:0,lineHeight:1}}
            title={playing ? "Pause animation" : "Play animation"}>
            {playing ? "❚❚" : "▶"}
          </button>
          <div style={{display:"flex",gap:4,flex:1,alignItems:"center"}}>
            {frames.map((f, i) => {
              const fOffset = baseTime ? Math.round((f.time - baseTime) / 60) : 0;
              const isCurrent = i === safeIdx;
              const fColor = fOffset <= 0 ? "#00cc66" : fOffset >= 30 ? "#ff7733" : fOffset >= 20 ? "#ff9933" : "#ffbb33";
              return (
                <button key={i} onClick={() => { setPlaying(false); setFrameIdx(i); }}
                  style={{flex:1,height:isCurrent?6:3,background:isCurrent?fColor:"#1a3050",border:"none",borderRadius:2,cursor:"pointer",padding:0,transition:"height 0.15s"}}
                  title={fOffset <= 0 ? "Now (live radar)" : `Forecast: in ${fOffset} minutes`}/>
              );
            })}
          </div>
          <span style={{color:"#3a5a78",fontFamily:"monospace",fontSize:9,whiteSpace:"nowrap"}}>
            {isLive ? "now" : `+${offsetMin}m`} of {Math.round((frames[frames.length-1].time - baseTime) / 60)}m
          </span>
        </div>
      )}

      {/* Legend */}
      <div style={{position:"absolute",bottom:6,left:8,right:8,background:"rgba(3,6,14,0.92)",borderRadius:5,padding:"5px 8px",border:`1px solid #0a1a28`}}>
        <div style={{color:"#0a1e30",fontFamily:"monospace",fontSize:8}}>
          {showCone
            ? `${isLive ? "LIVE RADAR" : timeLabel + " FORECAST"} + AI TRACK · 🔴 Direct  🟠 High  🟡 Possible`
            : hasForecast
              ? `RAINVIEWER NEXRAD · LIVE → +${Math.round((frames[frames.length-1].time - baseTime) / 60)} MIN FORECAST · click bars to scrub`
              : "LIVE RADAR · RainViewer NEXRAD · No forecast frames available right now"}
        </div>
      </div>
    </div>
  );
