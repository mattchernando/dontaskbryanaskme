...
function RadarMap({a, cities, radarHost, radarPath, showCone, zoom}) {
  ...
  return (
    <svg viewBox="-200 -170 400 340" style={{width:"100%",height:"100%",background:"#03060e"}}>
      ...
      {/* Header labels */}
      <text x="-195" y="-158" fill="#1a3a55" fontSize="6" fontFamily="monospace" filter="url(#labelShadow)">
        {visMi}-MI LIVE RADAR {showCone ? "• AI TRACK OVERLAY" : "• CLEAR CONDITIONS"}
      </text>
      <text x="-195" y="-148" fill={showCone ? col : "#1a4a60"} fontSize="5.5" fontFamily="monospace" filter="url(#labelShadow)">
        {showCone ? `MOTION: ${compass(a.stDir)} @ ${a.stSpd} MPH` : "NO SEVERE TRACK — MONITORING"}
      </text>
