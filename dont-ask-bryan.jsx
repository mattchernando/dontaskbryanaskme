...
export default function App() {
  ...
  const [radarData, setRadar] = useState({host:null,path:null});
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (past && past.length > 0) {
        ...
        setRadar({ host: data.host || "https://tilecache.rainviewer.com", path: latest.path });
        radarOk = true;
      }
    } catch(e) {
...
function MapPanel({a, impact, col, mob, radarData}) {
  ...
  return (
    <div style={{background:"#03060e",border:`1px solid ${a.hasTW?"#ff220050":S.border}`,borderRadius:mob?0:8,overflow:"hidden",position:"relative",minHeight:mob?280:360}}>
      <RadarMap a={a} cities={impact} radarHost={radarData?.host} radarPath={radarData?.path} showCone={showCone} zoom={zoom}/>
      ...
      <div style={{position:"absolute",top:10,left:8,background:"rgba(3,6,14,0.85)",borderRadius:4,padding:"3px 7px",border:"1px solid #0a1a28"}}>
      ...
      <div style={{position:"absolute",bottom:6,left:8,right:8,background:"rgba(3,6,14,0.92)",borderRadius:5,padding:"5px 8px",border:`1px solid #0a1a28`}}>
        <div style={{color:"#0a1e30",fontFamily:"monospace",fontSize:8}}>
          {showCone
            ? "LIVE RADAR + AI TRACK · Bunkers RM projection · 🔴 Direct  🟠 High  🟡 Possible"
            : "LIVE RADAR · RainViewer NEXRAD composite · Scroll zoom to adjust range"}
        </div>
      </div>
    </div>
  );
